...
- **Transactional Consistency**: Changes are handled through autogenerated scripts that can be reviewed and rolled back.
...
3. **Rollback**: Revert the last change if something went wrong.
   ...
   make migrate-undo
   ```
4. **Status**: View history and current version.
   ```bash
   make migrate-status
   ```
