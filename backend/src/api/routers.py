...
def toggle_task_completion(task_id: int, service: TaskService = Depends(get_task_service)):
...
def reorder_tasks(request: ReorderRequest, service: TaskService = Depends(get_task_service)):
